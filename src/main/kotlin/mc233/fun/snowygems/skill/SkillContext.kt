...
data class SkillContext(
    /** 触发技能的玩家 */
    val player: Player,
    /** 触发时手持(或发射瞬间记录)的物品, 技能可以读它的 lore 取数值, 也可以改它的附魔 */
    val item: ItemStack,
    /** 被解析后的这一行技能 */
    val line: SkillLine,
    /** 触发类型, 如 onUse / onShiftSwing / onHit:ARROW */
    val trigger: String,
    /** 被攻击/被命中的实体, 没有则为 null */
    val victim: LivingEntity? = null,
    /** 命中位置(发射物命中方块或实体的坐标), 没有则为 null */
    val hitLocation: Location? = null
) {

    /** 按 @target 解析作用目标: @Entity 作用于受害者, 其余作用于自己 */
