 ...
 * 例如 1.21.4 -> 12104, 1.21.11 -> 12111这样可以直接用整数比较, 不需要解析字符串
 *
 * ⚠️ 关于 26.1 / 26.2 这类新命名方案:
 * Mojang 从 2026 年起改用「年份.次版本」命名TabooLib 的 versionId 仍按它自己的解析规则给值,
 * 无法假定它会把 26.1 算成 260100 还是别的因此本插件**不依赖版本号做任何功能判定**——
 * 所有"这个版本有没有矛/铜盔甲/某个属性"的问题, 都直接问服务端注册表(见 [Registries])
 *
 * 本对象因此只剩「展示」职责: 日志和 /sgem compat 里报告服务端版本
 * 真正需要按 API 差异分支的地方(AttributeModifier 的新旧构造)走反射探测, 见 [AttributeCompat]
 */
