 ...
 *   本就该静默跳过所以只在 Debug 日志里记, 不打扰服主
 * - [Level.MISTAKE] 名字拼错、函数不存在、漏写触发标记这类**永远不会生效**, 必须报出来
 *
...
object ConfigValidator {
    ...
    data class Issue(val level: Level, val where: String, val what: String, val detail: String) {
        ...
        /** 报告用的一行文本 —— 三种输出级别共用同一个格式 */
        val line get() = "[$where] $what -> $detail"
    }
    ...
    /**
     * 三种"要查注册表的 Rewards 参数"的规格表.
     *
     * 属性/附魔/效果的校验逻辑一模一样(解析不出来 -> 判定是版本问题还是笔误), 唯一的差别就是
     * 查哪张表所以收成一张表, 加新的一类只需在这里加一行
     *
     * @param arg 要检查的参数名
     * @param resolve 正常解析路径(带模块门禁), 返回非 null 即通过
     * @param keysOf 该写法对应的候选注册表键
    ...
    private class Spec(
        val arg: String,
        val kind: String,
        val resolve: (String) -> Any?,
        val keysOf: (String) -> List<String>,
        val moduleOf: (String) -> String?,
        val existsRaw: (String) -> Boolean
    )

    private val SPECS: Map<String, Spec> = buildMap {
        val attribute = Spec(
            "name", "属性", Registries::attribute,
            { AttributeAliases.candidatesOf(it) ?: listOf(it.lowercase()) },
            FeatureModules::moduleOfAttribute, Registries::hasAttributeRaw
        )
        val enchant = Spec(
            "name", "附魔", Registries::enchantment,
            { listOf(EnchantAliases.keyOf(it) ?: it.lowercase()) },
            FeatureModules::moduleOfEnchantment, Registries::hasEnchantmentRaw
        )
        val effect = Spec(
            "type", "药水效果", Registries::effect,
            { listOf(EffectAliases.keyOf(it) ?: it.lowercase()) },
            FeatureModules::moduleOfEffect, Registries::hasEffectRaw
        )
        put("attribute", attribute)
        put("enchant", enchant)
        put("potion", effect)
        put("potionbuff", effect)
    }
    ...
    private fun validateRewardLine(where: String, raw: String) {
        ...
        val value = call.args[spec.arg] ?: return
        if (spec.resolve(value) != null) return

        val keys = spec.keysOf(value)
        val what = "${call.name}{${spec.arg}=$value}"
        val module = keys.firstNotNullOfOrNull(spec.moduleOf)
        when {
            module != null -> version(where, what, "${spec.kind}属于 $module 模块, 当前服务端没有或该模块已停用")
            keys.any(spec.existsRaw) -> version(where, what, "${spec.kind}被停用的模块挡下")
            else -> mistake(
                where, what,
                "当前版本 ${ServerVersion.minecraftVersion} 的注册表里没有这个${spec.kind}, 请检查拼写"
            )
        }
    }
    ...
    private fun report() {
        // 版本跳过是正常状态(同一份配置发到不同版本的子服), 只给一行概要 + Debug 明细
        versionSkips().takeIf { it.isNotEmpty() }?.let { skips ->
            info("配置中有 ${skips.size} 处内容当前版本不支持, 已跳过(属正常, 用 /sgem compat all 查看明细)")
            skips.forEach { DebugUtil.log("Compat", "  [跳过] ${it.line}") }
        }
        val errors = mistakes()
        if (errors.isEmpty()) {
        ...
        when (FeatureModules.onMissingFeature) {
            ...
            "warn" -> report(::warning, header, errors)
                ...
                report({ DebugUtil.log("Compat", it) }, null, errors)
            }
        }
    }

    private fun report(out: (String) -> Unit, header: String?, errors: List<Issue>) {
        header?.let(out)
        errors.forEach { out("  ${it.line}") }
    }
}
